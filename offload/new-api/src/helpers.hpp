...
offload_error_code_t getInfoImpl(size_t ParamValueSize, void *ParamValue,
                                 size_t *ParamValueSizeRet, T Value,
                                 size_t ValueSize, Assign &&AssignFunc) {
  if (!ParamValue && !ParamValueSizeRet) {
    return OFFLOAD_ERROR_CODE_INVALID_NULL_POINTER;
  }
  ...
  return OFFLOAD_ERROR_CODE_SUCCESS;
}

template <typename T>
offload_error_code_t getInfo(size_t ParamValueSize, void *ParamValue,
                             size_t *ParamValueSizeRet, T Value) {
...
template <typename T>
offload_error_code_t getInfoArray(size_t array_length, size_t ParamValueSize,
...
public:
  ...
  // Scalar return Value
  template <class T> offload_error_code_t operator()(const T &t) {
    return getInfo(ParamValueSize, ParamValue, ParamValueSizeRet, t);
  }

  // Array return Value
  template <class T> offload_error_code_t operator()(const T *t, size_t s) {
